package me.mavis.essentials;

public class Details {
    public static final String somedetail = "config";
    public static final String senderError = "senderError";
    public static final String notEnoughAgruments = "syntaxError.notEnoughArguments";
    public static final String notSupportYet = "syntaxError.notSupportYet";
    public static final String notHaveThatEnchantment = "syntaxError.notHaveThatEnchantment";
    public static final String numberArgument = "invalidArgument.numberArgument";
    public static final String outOfRange = "invalidArgument.outOfRange";
}
